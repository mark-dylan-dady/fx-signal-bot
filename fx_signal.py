...
def upload_image_to_imgur(image_path):
    """
    ローカルで生成したチャート画像をImgurへアップロードし、
    LINE APIが読み取れるHTTPS直リンクURLを取得する
    """
...
def send_line_notification(message, image_path=None):
    """
    最新のLINE Messaging API（Push Message）を使用して
    テキストメッセージとチャート画像を即時送信する
    """
    ...
    # 画像パスが与えられている場合、Imgurにアップロードして直リンクURLを取得
    if image_path:
        print("チャート画像を生成・アップロード中...")
        ...
        if uploaded_url:
            ...
            print("画像のURL化に失敗したため、テキストのみ送信します。")
    ...
    try:
        ...
        if response.status_code == 200:
            print("LINEへの最新チャート画像添付・送信に成功しました。")
            return True
        else:
...
if latest_action_val != 0 and not pd.isna(latest_action_val):
    ...
    # 生成されたローカル画像（trading_chart.png）を直接渡して送信
    send_line_notification(msg, image_path=chart_filename)
else:
